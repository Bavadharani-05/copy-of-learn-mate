...
export function CharacterSetup({ setView, setUser, setLearnerProfile }) {
  ...
  const ages = [
    ...
    { name: "Elder", emoji: "👩‍💼", char: "Lifelong Learner", icon: "award" },
  ...
  return (
    <div className="w-full max-w-3xl mx-auto py-6 animate-slide-up">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl font-display font-extrabold text-white mt-1">
              ...
              {step === 3 && "What would you enjoy doing most?"}
              {step === 4 && "Which age group are you in?"}
              {step === 5 && "How do you like learning?"}
            </h1>
          </div>
      ...
      <div className="bg-slate-900/50 border border-slate-850 p-6 md:p-8 rounded-3xl relative overflow-hidden">
        ...
        {step === 0 && (
          <div className="space-y-8 py-4 text-center">
            <div className="w-20 h-20 rounded-3xl bg-brand-500/15 border border-brand-500/20 flex items-center justify-center mx-auto shadow-lg shadow-brand-500/10 animate-float">
              <LucideIcon name="sparkles" className="w-10 h-10 text-brand-400" />
            </div>
            <div className="space-y-3">
              ...
              <p className="text-slate-400 text-sm md:text-base max-w-xl mx-auto leading-relaxed">
                Tell us a little about your preferences so we can make your learning experience feel more like you. No scientific claims—just pure personalized magic.
              </p>
            </div>
            ...
            <div className="pt-6 max-w-xs mx-auto">
              <button
                type="button"
                onClick={() => setStep(1)}
                className="w-full bg-brand-600 hover:bg-brand-500 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-lg hover:shadow-brand-500/25 flex items-center justify-center gap-2 text-sm"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {colors.map((c) => (
              <button
                key={c.name}
                type="button"
                onClick={() => handleSelect('color', c.name)}
                className={`p-6 rounded-2xl border text-left flex items-center gap-4 transition-all hover:scale-[1.02] bg-slate-950/60 border-slate-800 hover:border-slate-700`}
              >
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white ${c.class}`}>
                  <LucideIcon name={c.icon} className="w-6 h-6" />
                </div>
                <div>
                  <div className="font-bold text-slate-100 text-lg flex items-center gap-1.5">
                    {c.name}
                  </div>
                  <div className="text-xs text-slate-500 mt-0.5">{c.char}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {places.map((p) => (
              <button
                key={p.name}
                type="button"
                onClick={() => handleSelect('place', p.name)}
                className="p-6 rounded-2xl border text-left flex items-center gap-4 transition-all hover:scale-[1.02] bg-slate-950/60 border-slate-800 hover:border-slate-700"
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">{p.char}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {activities.map((a) => (
              <button
                key={a.name}
                type="button"
                onClick={() => handleSelect('activity', a.name)}
                className="p-6 rounded-2xl border text-left flex items-center gap-4 transition-all hover:scale-[1.02] bg-slate-950/60 border-slate-800 hover:border-slate-700"
              >
                <div className="w-12 h-12 rounded-xl bg-slate-900 border border-slate-850 flex items-center justify-center text-2xl">
                  {a.emoji}
                ...
                <div>
                  <div className="font-bold text-slate-100 text-lg">{a.name}</div>
                  <div className="text-xs text-slate-500 mt-0.5">{a.char}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="pt-6 border-t border-slate-850 flex justify-end">
              <button
                ...
                className={`py-3.5 px-8 rounded-xl font-bold flex items-center gap-2 text-sm shadow-md transition-all ${selections.learningPreference ? "bg-brand-600 hover:bg-brand-500 text-white hover:shadow-brand-500/20" : "bg-slate-800 text-slate-650 cursor-not-allowed"}`}
              >
          ...
          <div className="py-4 space-y-8 animate-fade-in text-center">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-xl mx-auto pt-4 text-left">
              <div className="bg-slate-950/80 border border-slate-850 p-4 rounded-2xl">
                ...
                <span className="text-xs font-semibold text-slate-200 block capitalize">{selections.learningPreference.replace('-', ' ')}</span>
              </div>
              <div className="bg-slate-950/80 border border-slate-850 p-4 rounded-2xl">
              ...
              <div className="bg-slate-950/80 border border-slate-850 p-4 rounded-2xl">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {computedProfile.traits.slice(0, 2).map(t => (
